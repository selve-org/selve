...
async def get_results(session_id: str):
    ...
    - Uses distributed Redis lock to prevent duplicate OpenAI calls
    - Handles concurrent requests gracefully
    - Catch-all UniqueViolationError handling in save_result
    ...
    # 🔒 DISTRIBUTED LOCK: Prevent duplicate OpenAI calls on concurrent requests
    # If another request is already generating results for this session,
    # wait for it to finish and then return the cached result
    lock_name = f"results:{session_id}"
    lock_token = redis_store.acquire_lock(
        lock_name=lock_name,
        lock_timeout=180,  # 3 minutes max for narrative generation
        blocking=True,
        blocking_timeout=200  # Wait up to 3.5 minutes for lock
    )
    
    if lock_token is None:
        # Lock acquisition timed out - another request is taking too long
        # Check if results were created while we waited
        existing_result = await service.get_result(session_id)
        if existing_result:
            db_session = await service.get_session(session_id)
            demographics = db_session.demographics if db_session else {}
        ...
        raise HTTPException(
            status_code=503,
            detail="Results generation is taking longer than expected. Please try again in a moment."
        )
    
    # We have the lock - but check AGAIN if results were created while we waited
    # This handles the case where we waited for a lock and the other request succeeded
    existing_result = await service.get_result(session_id)
    if existing_result:
        # Results were created by another request while we waited for lock
        redis_store.release_lock(lock_name, lock_token)
        db_session = await service.get_session(session_id)
        demographics = db_session.demographics if db_session else {}
        validation_data = None
        if existing_result.consistencyScore is not None:
    ...
    # ==========================================================================
    # LOCKED SECTION: Generate new results (expensive OpenAI calls)
    # The lock ensures only ONE request generates results at a time
    # ==========================================================================
    try:
        # Get session
